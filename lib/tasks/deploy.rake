task :deploy => ['deploy:set_app', 'deploy:push', 'deploy:tag']

namespace :deploy do
  APP_NAME = 'young-woodland-1571'

  task :migrations => [:set_app, :push, :off, :migrate, :assets, :restart, :on, :tag]
  task :rollback => [:set_app, :off, :push_previous, :assets, :restart, :on]
  ...
  task :assets do
    ...
    puts `heroku run rake assets:precompile --app #{APP}`
  end
